...
export function App() {
  ...
  const sqlOpen = useStore((s) => s.sqlOpen)
    ...
    <div className="flex h-full flex-col">
      <header className="chrome flex h-9 shrink-0 items-center gap-3 border-b border-[var(--color-border)] bg-[var(--color-panel)] px-3">
        ...
        <button
          ...
          className="rounded border border-[var(--color-border-strong)] px-2 py-0.5 text-[11px] text-[var(--color-muted)] hover:border-[var(--color-accent)] hover:text-[var(--color-text)]"
        >
          Ctrl+K
        </button>
        {activeRef && (
          <span className="truncate font-[var(--font-mono)] text-[12px] text-[var(--color-muted)]">
...
function EmptyState() {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-3 text-[var(--color-faint)]">
      <p className="text-[15px]">Nothing open</p>
      <p>
        Press{' '}
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (mod && e.key.toLowerCase() === 'f') {
        e.preventDefault()
        if (s.sqlOpen) s.setSqlOpen(false)
